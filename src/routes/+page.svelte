...
<section id="intro" class="intro">
  ...
  <!-- social icon row -->
  <div class="socials">
    <a class="icon" href={profile.github} target="_blank" rel="noreferrer" aria-label="GitHub" title="GitHub" innerHTML={Icon.github} />
    <a class="icon" href={profile.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" title="LinkedIn" innerHTML={Icon.linkedin} />
    <a class="icon" href={profile.x} target="_blank" rel="noreferrer" aria-label="X" title="X" innerHTML={Icon.x} />
  </div>
</section>
...
<section id="teams">
  ...
  <div class="cards">
    {#each experience as item}
      <div class="card exp">
        {#if item.logo}
          <img class="logo" src={item.logo} alt={`${item.org} logo`} />
        {/if}
        <div class="exp-text">
...
<section id="testimonials">
  ...
  <div class="cards">
    {#each testimonials as t}
      <figure class="card">
        {#if t.avatar}<img class="avatar small" src={t.avatar} alt={t.name} />{/if}
        <figcaption>“{t.quote}” — {t.name}{#if t.title}, {t.title}{/if}</figcaption>
      </figure>
    {/each}
  </div>
</section>
...
<style>
  ...
  .icon { display:inline-flex; align-items:center; justify-content:center; width:34px; height:34px; border-radius:8px; background:#fff; color:#111; border:1px solid #e2e8f0; transition:.15s; }
  ...
  /* small avatar for testimonials if used later */
